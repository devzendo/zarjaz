...
public class TestTransports extends ConsoleLoggingUnittestCase {
    ...
    public interface PrimitiveArgumentMethod {
        public int question(boolean b, byte by, char c, short s, int i, long l, float f, double d);
    }

    @Test
    // bug fix test: was getting the classes of the arguments to the method (in the nulltransport transport proxy)
    // rather than the classes of the method's actual parameters, and autoboxing was passing an Integer where the
    // method actually required an int.
    public void methodsWithPrimitiveArgumentsAreCorrectlyDispatched() {
        final PrimitiveArgumentMethod pam = new PrimitiveArgumentMethod() {
            @Override
            public int question(final boolean b, final byte by, final char c, final short s, final int i, final long l, final float f, final double d) {
                return (b ? 1 : 0) + by + c + s + i + (int) l + (int) f + (int) d;
            }
        };
        serverTransport.registerServerImplementation(endpointName1, PrimitiveArgumentMethod.class, pam);

        final PrimitiveArgumentMethod client1 = clientTransport.createClientProxy(endpointName1, PrimitiveArgumentMethod.class, METHOD_TIMEOUT_MILLISECONDS);

        clientTransport.start();

        assertThat(client1.question(true, (byte) 1, 'a', (short) 3, 5, 4L, 2.3f, 4.1d), equalTo(117));
    }
    ...
    @Test
    public void canCreateClientProxyForSameEndpointNameTwice() {
        // can't see why you'd want to, but you can - the sequence number generated by each client of the same
        // transport will be different (the transport holds the sequence count).
        final Incrementer server = new Incrementer() {
            @Override
            public int increment(final int number) {
                return number + 1;
            }
        };
        serverTransport.registerServerImplementation(endpointName1, Incrementer.class, server);

        final Incrementer client1 = clientTransport.createClientProxy(endpointName1, Incrementer.class, METHOD_TIMEOUT_MILLISECONDS);
        final Incrementer client2 = clientTransport.createClientProxy(endpointName1, Incrementer.class, METHOD_TIMEOUT_MILLISECONDS);

        clientTransport.start();

        assertThat(client1.increment(5), equalTo(6));
        assertThat(client2.increment(17), equalTo(18));
        assertThat(client1.increment(2), equalTo(3));
        assertThat(client2.increment(21), equalTo(22));
    }
}
